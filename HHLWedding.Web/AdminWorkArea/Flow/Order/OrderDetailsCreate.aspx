<%@ Page Title="跟单沟通" Language="C#" MasterPageFile="~/AdminWorkArea/Master/PopuMaster.Master" AutoEventWireup="true" CodeBehind="OrderDetailsCreate.aspx.cs" Inherits="HHLWedding.Web.AdminWorkArea.Flow.Order.OrderDetailsCreate" %>
        ...
        $(function () {
            ...
            $("input[name='rdoState']").change(function () {
                if ($(this).val() == 6) {           //二选一
                    ...
                    $(".tr_nextDate").find("td").eq(0).text("下次沟通时间:");
                    $(".tr_orderEmp").addClass("hide");
                    $(".tr_nextDate").show();
                } else if ($(this).val() == 7) {    //多选一
                    $(".span-content").text("沟通内容:");
                    $(".tr_nextDate").find("td").eq(0).text("下次沟通时间:");
                    $(".tr_orderEmp").removeClass("hide");
                    $(".tr_nextDate").show();
                } else if ($(this).val() == 8) {      //成功预定
                    $(".span-content").text("沟通内容:");
                    $(".tr_nextDate").find("td").eq(0).text("提案时间:");
                    $(".tr_orderEmp").addClass("hide");
                    $(".tr_nextDate").show();
                } else if ($(this).val() == 9) {      //流失(跟单)
            ...
            $("#btnConfirm").click(function () {
                ...
                var content = $("#txtOrderContent").val();
                var nextDate = $("#txtOrderNextFollowDate").val()
                ...
                hhl.ajax("/WebService/Flow/OrderHandler.asmx/OrderCotentCreate", order, function (result) {
                    if (result.d.IsSuccess) {
                        SaveCustomerInfo();
                        hhl.notify.success(result.d.Message, "提示");
                        setTimeout(function () {
                            if (state == 6 || state == 7) {       //二选一或多选一
                                window.location.href = "FollowOrderList.aspx";
                            } else if (state == 8) {        //成功预订
                                window.location.href = "SuccessOrderList.html";
                            } else if (state == 9) {        //流失
                                window.location.href = "LoseOrderList.html";
                            }
                        }, 1200);
                    } else {
            ...
            function SaveCustomerInfo() {
                ...
                hhl.ajax("/WebService/Flow/CustomerHandler.asmx/ModifyCustomer", customer, function () { }, 1);
            }
        });
    ...
    <div class="panel">
        <div class="panel-body">
            <div class="widget-box">
                            ...
                            <ItemTemplate>
                                ...
                                <tr class="tr-inviteContent">
                                    <td style="border-bottom: 1px dotted gray;">下次沟通日期:&nbsp;&nbsp;&nbsp;&nbsp;
                                        <%#Eval("NextFollowDate","{0:yyyy-MM-dd}").ToString() == "" ? "无" : Eval("NextFollowDate","{0:yyyy-MM-dd}") %> 
                                    </td>
                                </tr>
                            </ItemTemplate>
                        </asp:Repeater>
                ...
                <!--销售跟单-->
                <div class="widget-header">
                    <h4 class="h-title pull-left"><i class="icon-reorder"></i><b>销售跟单</b></h4>
                    <div class="box box1 pull-right">
                        <div class="div_angle">
                            <span class="span-angel">
                                <i id="icon_angle2" class="icon-angle icon-angle-up"></i>
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="widget-content" style="height: auto;">
                    <table id="tblOrderContent" class="table table-bordered table-no-border tblSystem col-width-4">
                        <tr>
                            <td style="width: 100px;">沟通状态:</td>
                            <td>
                                <input type="radio" name="rdoState" class="input_radio" value="6" checked="checked" />二选一
                                <input type="radio" name="rdoState" class="input_radio" value="7" />多选一
                                <input type="radio" name="rdoState" class="input_radio" value="8" />成功预订
                                <input type="radio" name="rdoState" class="input_radio" value="9" />流失
                            </td>
                        </tr>
                        <tr>
                            <td><span class="span-content">沟通内容:</span></td>
                            <td>
                                <textarea id="txtOrderContent" cols="45" rows="5" style="max-width: 338px; max-height: 96px;"></textarea></td>
                        </tr>
                        <tr class="tr_nextDate">
                            <td>下次沟通时间:</td>
                            <td>
                                <input class="form-control txtDate" id="txtOrderNextFollowDate" type="text" />
                            </td>
                        </tr>
                        <tr>
                            <td colspan="2">
                    ...
                    <table class="table tbl_noborder tblSystem">
                        ...
                        <tr class="tr_NoOrder" id="tr_noOrder" runat="server">
                            <td colspan="3">
                                <h4>暂无沟通记录</h4>
                            </td>
                        </tr>
                        <asp:Repeater runat="server" ID="repOrderContent" ClientIDMode="Static">
                            <ItemTemplate>
                                <tr class="tr-orderContent">
                                    <td>第<%#Container.ItemIndex+1 %>次沟通&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                        沟通时间：<%#Eval("CreateDate") %>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                ...
                                <tr class="tr-orderContent">
                                    <td>
                                        <%#Eval("OrderState").ToString() == "4" ? "流失原因(内容)" : "沟通内容" %>:&nbsp;&nbsp;&nbsp;&nbsp;
                                          <span class="span_content" style="white-space: normal; width: 200px">
                                              <%#Eval("OrderContent") %>
                                          </span>
                                    </td>
