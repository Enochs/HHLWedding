<%@ Page Title="" Language="C#" MasterPageFile="~/AdminWorkArea/Master/PopuMaster.Master" AutoEventWireup="true" CodeBehind="HotelUpdate.aspx.cs" Inherits="HHLWedding.Web.AdminWorkArea.Foundation.Hotel.HotelUpdate" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
        ...
        <div class="panel-body col-md-12">
            <table id="tbl-border" class="table table-bordered">
                <tr>
                    ...
                    <td>酒店类型</td>
                    <td>
                        <%--<select id="selHotelType" class="form-control">
                            <option selected="selected" value="5">五星级酒店</option>
                            <option value="4">四星级酒店</option>
                            <option value="3">三星级酒店</option>
                            <option value="2">自助餐厅</option>
                            <option value="1">特色酒店</option>
                        </select>--%>
                        <asp:DropDownList runat="server" ID="selHotelType" ClientIDMode="Static" CssClass="form-control" DataTextField="Text" DataValueField="Value" />
                    </td>
                </tr>
                <tr>
                    <td>区域</td>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            hhl.ajax("/WebService/HotelHandler.asmx/GetHotelById", JSON.stringify({ hotelId: hotelId }), function (result) {
                console.log(result);
                if (result.d.IsSuccess) {
                    var data = result.d.Data;
                    $("#txtHotelName").val(data.HotelName);
                    $("#txtArea").val(data.Area);
                    $("#txtAddress").val(data.Address);
                    $("#txtPhone").val(data.Phone);
                    $("#txtDeskCount").val(data.DeskCount);
                    $("#txtStartMoney").val(data.Start);
                    $("#txtEndMoney").val(data.End);
                    $("#txtSort").val(data.Sort);
                    $("#selHotelType").val(data.HotelType);
                ...
                var chks = $("#dlLabel").find("input[name='chkLabel']");
                if (data.Label.length > 0) {
                    for (var i = 0; i < data.Label.length; i++) {
                        var id = data.Label[i];
                        if (id != ",") {
                            $("#dlLabel #" + id).find("input[name='chkLabel']").prop("checked", "checked");
                        }
                    }
                }
                }
            ...
            function submitForm() {
                ...
                if (validate()) {
                    ...
                    var type = $("#selHotelType option:selected").val();        //酒店类型


                    var hotel = '{"hotel":{"HotelID":"' + hotelId + '","HotelName":"' + $("#txtHotelName").val() + '","HotelType":"' + $("#selHotelType option:selected").val() + '","Area":"' + $("#txtArea").val() + '","Address":"' + $("#txtAddress").val() + '","Phone":"' + $("#txtPhone").val() + '","Start":"' + $("#txtStartMoney").val() + '","End":"' + $("#txtEndMoney").val() + '","DeskCount":"' + $("#txtDeskCount").val() + '","Description":"' + ue.getContent() +
                      '","Sort":"' + $("#txtSort").val() + '","Label":"' + labelId + '","LabelContent":"' + lableContent + '"}}';

                    hhl.ajax("/WebService/HotelHandler.asmx/UpdateHotel", hotel, hhl.callbackRefresh);
                }
            }
