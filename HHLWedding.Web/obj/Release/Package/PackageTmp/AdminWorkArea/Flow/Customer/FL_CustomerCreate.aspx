<%@ Page Title="" Language="C#" MasterPageFile="~/AdminWorkArea/Master/PopuMaster.Master" AutoEventWireup="true" CodeBehind="FL_CustomerCreate.aspx.cs" Inherits="HHLWedding.Web.AdminWorkArea.Flow.Customer.FL_CustomerCreate" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $(".sel_customerState").val("5");

            $(".txtBride").val("alice");
            $(".txtBridePhone").val("15883296851");
            $("#partyDate").val("2016-09-28");
            $("#ddlHotel").val("2");
            $("#ddlSaleType").val("2");
            $("#ddlSaleSource").val("2");
            $(".txtDescription").val("添加的客户说明 默认 测试");
                        ...
                        if (phone != "" && phone != undefined) {
                            if (isRegPhone().test(phone)) {
                                if (res == false) {
                                    ...
                                    hhl.ajax("/WebService/Flow/CustomerHandler.asmx/IsExistsPhone", '{"phone":' + phone + '}', function (result) {
                                        resNum = result.d;
                                        if (result.d == 1) {
                                            message = "号码已经存在";
                                            res = false;
            ...
            $("#btnSave").on("click", function () {
                ...
                //电话验证通过
                if (res[0]) {
                    var info = '"Bride":"' + $(".txtBride").val() + '","Groom":"' + $(".txtGroom").val() + '","Operator":"' + $(".txtOperator").val() + '","BridePhone":"' + $(".txtBridePhone").val() + '","GroomPhone":"' + $(".txtGroomPhone").val() + '","OperatorPhone":"' + $(".txtOperatorPhone").val() + '"';
                    var customer = '{"custoer":{' + info + ',"Hotel":"' + $("#ddlHotel option:selected").val() + '",SaleType":"' + $("#ddlSaleType option:selected").val() + '","Channel":"' + $("#ddlSaleSource option:selected").val() + ',"Type":"' + $(".sel_Type").val() + '","State":"' + $(".sel_customerState").val() + '","IsVip":"' +
                        $(".rdoIsVip:checked").val() + '","BanqueType":"' + $(".rdoBanqueType:checked").val() + '","Description":"' + $(".txtDescription").val() + '"}}'
                    hhl.ajax("/WebService/Flow/CustomerHandler.asmx/CreateCustomer", customer, function (result) {
                        if (result.d.IsSuccess) {
    ...
    <div class="panel">
        <div class="widget-box">
            ...
            <div class="widget-content">
                ...
                <table id="tblSystem" class="table table-bordered" style="width: 48%; margin-left: 10px;">
                    <tr>
                        <td>客户状态</td>
                        <td>
                    ...
                    <tr>
                        <td class="td_cusType" colspan="4">
                            <input type="checkbox" class="checkbox-inline chkCus chkBride" checked="checked" /><span class="span_bride">新娘</span>
                            <input type="checkbox" class="checkbox-inline chkCus chkGroom" /><span class="span_groom">新郎</span>
                    ...
                    <tr>
                        <td colspan="4">
                            <button type="button" id="btnSave" class="btn btn-success btn-sm">添加</button>
                            <button type="reset" id="btnClear" class="btn btn-danger btn-sm">清空</button>
                        </td>
                    </tr>
                </table>
