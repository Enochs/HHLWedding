...
namespace HHLWedding.BLLAssmbly
{
    public class EmployeeType : ICRUDInterface<Sys_EmployeeType>
    {
        ...
        {
            return ObjEntity.Sys_EmployeeType.ToList();
        }
        /// <summary>
        /// 根据状态获取员工类型
        /// </summary>
        public List<Sys_EmployeeType> GetAllByStatus(byte Status)
        {
            return ObjEntity.Sys_EmployeeType.Where(C => C.Status == Status).ToList();
        }
        ...
        public List<Sys_EmployeeType> GetAllByPage(int pageIndex, int pageSize, List<Expression<Func<Sys_EmployeeType, bool>>> parmList, string sort, out int count)
        {
