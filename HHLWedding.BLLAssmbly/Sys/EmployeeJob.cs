...
namespace HHLWedding.BLLAssmbly
{
    public class EmployeeJob : ICRUDInterface<Sys_EmployeeJob>
    {
        ...
        public int Delete(Sys_EmployeeJob ObjectT)
        {
            if (ObjectT != null)
            {
                ...
                ObjEntity.Sys_EmployeeJob.FirstOrDefault(
                   C => C.JobID == ObjectT.JobID).Status = 0;
        ...
        {
            return ObjEntity.Sys_EmployeeJob.ToList();
        }

        /// <summary>
        /// 根据状态获取员工职务
        /// </summary>
        public List<Sys_EmployeeJob> GetAllByStatus(byte Status)
        {
            return ObjEntity.Sys_EmployeeJob.Where(C => C.Status == Status).ToList();
        }
        /// <summary>
        /// 根据主键返回单个员工工作信息
        /// </summary>
        ...
        public List<Sys_EmployeeJob> GetAllByPage(int pageIndex, int pageSize, List<Expression<Func<Sys_EmployeeJob, bool>>> parmList, string sort, out int count)
        {
